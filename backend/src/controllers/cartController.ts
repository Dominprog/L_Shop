...
export function addToCart(req: Request, res: Response): void {
  ...
  if (!productId || !quantity || quantity < 1) { res.status(400).json({ error: 'Invalid data' }); return; }
  ...
  if (newQty > product.stock) {
    res.status(400).json({ error: `Максимум ${product.stock} шт.` }); return;
  }

  if (existing) existing.quantity = newQty;
  else users[userIndex].cart.push({ productId, quantity });
...
export function updateCartItem(req: Request, res: Response): void {
  ...
  if (!item) { res.status(404).json({ error: 'Item not found in cart' }); return; }
